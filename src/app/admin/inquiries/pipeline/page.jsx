  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">CRM Kanban Pipeline</h1>
          <p className="text-gray-500 text-sm mt-1">Drag and drop leads across statuses.</p>
        </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        <div className="flex gap-4 h-full min-w-max">
          {KANBAN_STAGES.map(stage => {
            ...
            return (
              <div key={stage} className="w-80 bg-gray-100 rounded-lg flex flex-col max-h-full">
                <div className="p-3 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-lg">
                  <h3 className="font-semibold text-gray-700 text-sm tracking-wide">{stage.replace('_', ' ')}</h3>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                  {stageInquiries.map(inquiry => (
                    <Link 
                      ...
                      className="block bg-white p-4 rounded-md shadow-sm border border-gray-200 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer group"
                    >
                      ...
                      <div className="font-medium text-gray-900 leading-tight mb-1">{inquiry.name}</div>
                      <div className="text-xs text-gray-500 mb-3">{inquiry.company || 'Individual'}</div>
                  ...
                  {stageInquiries.length === 0 && (
                    <div className="text-center p-6 text-sm text-gray-400 border-2 border-dashed border-gray-200 rounded-md">
                      No leads
                    </div>
                  )}
                </div>
