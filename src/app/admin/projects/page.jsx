    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage case studies and project portfolio.</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">Project Title</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {projects.map(project => (
              <tr key={project.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="py-4 px-6 text-sm text-gray-500">{project.client || 'N/A'}</td>
                <td className="py-4 px-6 text-sm text-gray-500">{project.category}</td>
                <td className="py-4 px-6">
                  <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${
                    project.status === 'Completed' ? 'bg-green-100 text-green-800' : 
                    project.status === 'DISABLED' ? 'bg-gray-100 text-gray-600' : 'bg-blue-100 text-blue-800'
                  }`}>
                    {project.status}
                  </span>
                </td>
